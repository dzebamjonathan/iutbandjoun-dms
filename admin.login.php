...
//define variables 
$email = $password = $email_error = $password_error = $username = "";
$errors = array();
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(count($errors)) {
        ...
        // find admin in database with the password and email
        $table =  'admins';
        $condition = "email = '$email'";
        $verified_admin = $db->selectWhere($table, $condition);
        ...
        if($verified_admin->password == $password) {
            // create a session for this user
            $_SESSION['active_admin'] = $verified_admin->id;
...
echo $twig->render('login.admin.twig', [
    'error' => $errors,
